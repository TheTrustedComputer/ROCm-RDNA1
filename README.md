...
- A decent computer running a Linux distribution
    ...
    - Intel Core 10th Gen
    - 32 GB RAM
- About 50 GB of free disk space for the full build
...
By default, all applications are stored in the ``$HOME/Docker/Apps`` directory under the Docker user ``rdna1_rocm-A.B_container`` for persistence, where ``A.B`` is the ROCm point release number. You can change these by editing the ``.env`` file in the ``SOFTWARE_DIR`` and ``HOME_USER`` fields.
...
### Debian 13: ROCm 6.4.2 + PyTorch 2.7.1 + ONNX Runtime 1.22.1

Provides newer software and libraries when performance isn't a priority. Choose this if you have conflicting dependencies, erratic program behavior, or runtime errors due to incompatible ABIs on the Debian 12 config.

### Arch Linux: Latest ROCm + PyTorch + ONNX Runtime

This distribution tends to have the latest and greatest of just about everything. Note that this config follows upstream and is mostly unaudited, so proceed at your own risk. You might want to give it a shot if you value living on the bleeding edge at the expense of the occasional bug. 
...
## Considerations
 ...
 - This PyTorch cannot use flash or memory-efficient attention for transformers because Triton currently does not implement support for RDNA1.
 ...
 - Composable kernels won't compile out of the box but will when patched to include them. This is mandatory when building PyTorch >= 2.6.
